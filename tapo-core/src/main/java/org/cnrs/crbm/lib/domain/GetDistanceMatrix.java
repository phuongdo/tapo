	...
	public PDPDistanceMatrix getDistanceMatrix(Atom[] protein)
		...
		for (i = 1; i < protein.length; i++) {
			for (j = i; j < protein.length - 1; j++) {
				/* beta-sheet */
				if (dist[i][j] >= 2 && j - i > 5) {
					if (dist[i - 1][j - 1] >= 2 && dist[i + 1][j + 1] >= 2
							|| dist[i - 1][j + 1] >= 2
							&& dist[i + 1][j - 1] >= 2) {
						dist[i][j] += 4;
						dist[j][i] += 4;
					...
					/* alpha-helices */
						...
						if (dist[i - 3][j - 3] >= 1 && dist[i + 3][j + 3] >= 1
								|| dist[i - 3][j + 3] >= 1
								&& dist[i + 3][j - 3] >= 1) {
							dist[i][j] += 4;
							dist[j][i] += 4;
							...
							if ((dist[i - 3][j - 3] >= 1
									|| dist[i - 3][j - 4] >= 1
									|| dist[i - 4][j - 3] >= 1 || dist[i - 4][j - 4] >= 1)
									&& (dist[i + 4][j + 4] >= 1
											|| dist[i + 4][j + 3] >= 1
											|| dist[i + 3][j + 3] >= 1 || dist[i + 3][j + 4] >= 1)
									|| (dist[i - 4][j + 4] >= 1
											|| dist[i - 4][j + 3] >= 1
											|| dist[i - 3][j + 4] >= 1 || dist[i - 3][j + 3] >= 1)
									&& (dist[i + 4][j - 4] >= 1
											|| dist[i + 4][j - 3] >= 1
											|| dist[i + 3][j - 4] >= 1 || dist[i + 3][j - 3] >= 1)) {
								dist[i][j] += 4;
