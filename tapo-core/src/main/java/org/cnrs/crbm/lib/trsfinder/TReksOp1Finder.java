...
public class TReksOp1Finder extends Finder {
    static Logger logger = LoggerFactory.getLogger(TReksOp1Finder.class);

    public TReksOp1Finder(Features features) {
        super(features);
        this.name = "TREKS-Op1";
    }
    ...
    public void findRepeat(Features features) {
        ...
        String seqAl = sequence3D.getSAV2NoInOutSide(features.getStrSeqAp());
        Seq3d seq3D = new Seq3d(seqAl);
        String pattern = seq3D.getSeq();
        List<Conformation> confors = seq3D.getConfors();
        // String pattern = this.features.getStrSeqAp();
        try {
