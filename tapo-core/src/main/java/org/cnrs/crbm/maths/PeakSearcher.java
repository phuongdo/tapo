 ...
 * <p>
 * This class search for peaks in the given vector. It searches the 
 * highest and the second highest peak too. The algorithm is based on the 
 * first derivation of the given float vector.
...
public class PeakSearcher {
     ...
     * @param float underborder - the under border of the amplitude for searching. Values under this number are ignored. (typical 0.2)
     * @param int beginsearchborder - defines the border in which the peaks are searched. FOr example: If at the beginning are some peaks that should not be founded this value can be set to 10, then the first 10 samples are jumped over.
     * @param int endsearchborder - defines the upper search border. See above
     */
    public PeakSearcher (float[] vector, float underborder, int beginsearchborder, int endsearchborder) {
        if (beginsearchborder<1) beginsearchborder = 1;
        localmaximas = new int[vector.length]; // Store for local maximas, 0=no local maxima,1=Beside, <1 local Max
        for(int i=beginsearchborder;i<(vector.length-endsearchborder-1);i++)           // Loop over complette vector
            if ((vector[i]>vector[i-1]) && (vector[i]>vector[i+1] && vector[i]>underborder)) {
                ...
                if (getVectorElement(vector,secondhighestpeakpos)<vector[i] && vector[highestpeakpos]>vector[i]) {
    ...
    /**
     * Returns an vector with the length of the float vector. Possible numbers of the 
     * vector are: 0 if ot is no local maximum; 1 if it is beside a local maximum; 2 and 
     * higher is it is a local maximum and higher numbers for other local maximas. For 
     * example the 5 nd highest maxima has 5 at the position of the array where the 5-nd
     * highest maxima is.
